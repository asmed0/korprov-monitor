...
var startMonitor = async function () {
  for (let y = 0; y < userConfig.locations.length; y++) {
    //placeholders
    var freshstart = true;
    var startValue;
    ...
    var locationID = await getLocationID(
      userConfig.personnummer,
      userConfig.locations[y].stad,
      userConfig.locations[y].postnummer,
      userConfig.anvandpostnummer
    );
      ...
      axios(config)
        .then(function (response) {
          ...
          if (freshstart === true) {
              ...
              .then(
                console.log(
                  `New time sent to webhook on ${
                    ...
                    response.data.data.bundles[0].occasions[0].locationName
                  } at ${
                    response.data.data.bundles[0].occasions[0].time
            ...
            console.log(
              "No new times availabe in " + userConfig.locations[y].stad
            );
          }
        })
        .catch(function (error) {
